...
class PreprocessConfig(BaseModel):
    ...
    autocrop: bool = False
    # Minimum span between the 99th and 1st percentile of the grayscale patch,
    # below which the region counts as blank. Otsu assumes two brightness
    # classes; on an unlit scoreboard cell there is only sensor noise, and it
    # splits that into a high-frequency pattern the engine reads digits from.
    # Measured on a live board: occupied cells land at 239-252, empty ones at
    # 10-29, so the default sits well clear of both. Zero disables the check.
    min_contrast: float = Field(default=60.0, ge=0.0, le=255.0)
